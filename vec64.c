...
PyDoc_STRVAR(vec64_pair_encode__doc__,
"Integer-encode index pairs from a sequence of Base64 symbol indexes.\n"
"\n"
"Given a bytes-like object of Base64 alphabet symbol indexes `s` such\n"
"as returned by `vectorize`, return a tuple of `len(s) // 2` integers\n"
"encoding the non-overlapping pairs of symbols in `s`.  Encoded pairs\n"
"are calculated by left-shifting the second symbol index in each pair\n"
"by six bits and then bitwise-ORing that value with the first symbol\n"
"index in the pair.  The algorithm is roughly equivalent to:\n"
"\n"
"```python\n"
"def pair_encode(s):\n"
...
static PyObject *
vec64_pair_encode(PyObject *self, PyObject *args, PyObject *kwargs)
{
    const char *symbols;
